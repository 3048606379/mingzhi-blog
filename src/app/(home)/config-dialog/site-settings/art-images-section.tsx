...
import type { ArtImageUploads } from './types'
...
export function ArtImagesSection({ formData, setFormData, artImageUploads, setArtImageUploads }: ArtImagesSectionProps) {
	...
	return (
		<div>
			<SectionHeader>ART_IMAGES</SectionHeader>
			...
			<div className='grid grid-cols-4 gap-3 max-sm:grid-cols-3'>
					...
					return (
						<div key={item.id} className='group relative'>
							<button
								...
								className={`block w-full overflow-hidden rounded-lg border bg-white/5 transition-all ${
							...
							<button
								...
								className='text-secondary absolute top-1 right-1 hidden rounded-full border bg-black/70 px-1.5 py-0.5 text-[10px] group-hover:block'
								style={{ borderColor: 'var(--color-border)' }}>
								删除
							</button>
						</div>
				...
				<div className='flex items-center justify-center'>
					<button
						...
						className='hover:border-brand/60 flex h-24 w-full items-center justify-center rounded-lg border border-dashed bg-transparent text-2xl text-gray-400 transition-colors hover:bg-white/5'
						style={{ borderColor: 'var(--color-border)' }}>
						+
					</button>
				</div>
