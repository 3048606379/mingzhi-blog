...
export function SocialButtonsSection({ formData, setFormData, socialButtonImageUploads, setSocialButtonImageUploads }: SocialButtonsSectionProps) {
	...
	return (
		<div>
			...
			{buttons.length === 0 && (
				<p className='mb-2 text-xs' style={{ color: '#555' }}>
					暂未配置社交按钮，点击下方「+」添加。
				</p>
			)}
			<div className='space-y-2 whitespace-nowrap'>
					...
					<div key={button.id} className='flex items-center gap-2'>
								...
								{socialButtonImageUploads[button.id]?.type === 'file' ? (
									...
									<>
										...
										<button
											...
											className='rounded-lg border bg-transparent px-3 py-1.5 text-xs transition-colors hover:border-[var(--color-brand)] hover:text-white'
						...
						<div className='flex gap-1'>
							<button
								...
								disabled={index === 0}
								className='rounded px-2 py-1 text-xs transition-colors hover:bg-[rgba(167,139,250,0.12)] hover:text-white disabled:cursor-not-allowed disabled:text-gray-600'>
								↑
							</button>
							<button
								type='button'
							...
							<button type='button' onClick={() => handleRemoveButton(button.id)} className='rounded px-2 py-1 text-xs text-red-400 transition-colors hover:bg-[rgba(239,68,68,0.12)]'>
								删除
							</button>
						</div>
					</div>
				...
				<button
					...
					className='hover:border-brand/60 text-secondary flex w-full items-center justify-center rounded-lg border border-dashed bg-transparent py-2 text-sm transition-colors hover:bg-white/5'
