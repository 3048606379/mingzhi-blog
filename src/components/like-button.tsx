	...
	if (show)
		return (
			<motion.button
				...
				onClick={handleLike}
				className={clsx('heartbeat-container relative overflow-visible border bg-transparent p-3 transition-colors hover:border-[var(--color-brand)]', className)}
				...
				{typeof count === 'number' && (
					<motion.span
						initial={{ scale: 0.4 }}
						animate={{ scale: 1 }}
						className='absolute -top-2 left-8 min-w-5 border bg-black px-1 py-0.5 text-center text-[10px] tabular-nums'
						style={{ borderColor: liked ? '#fb7185' : 'var(--color-border)', color: liked ? '#fb7185' : '#777' }}>
						{count}
					</motion.span>
				)}
